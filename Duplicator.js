...
if (typeof jQuery != "undefined") {
  $(document).ready(function () {
    ...
    $backupNowBtn.on("click", function (ev) {
      ...
      if (
        !$("input[name=useLocalFolder]").is(":checked") &&
        !$("input[name=useGoogleDrive]").is(":checked") &&
        !$("input[name=useDropbox]").is(":checked") &&
        !$("input[name=useFTP]").is(":checked") &&
        !$("input[name=useAmazonS3]").is(":checked")
      ) {
    ...
    $("#wrap_cronMode").on("click", function () {
      if ($("#cronMode_LazyCron").is(":checked")) {
        ...
        $("#cronMode_PWCron").is(":checked") ||
        $("#cronMode_none").is(":checked")
      ) {
        $("li#wrap_Inputfield_cycle").addClass("invisible");
      }
    });
