...
$(document).ready(function () {
  ...
  function redirectToDuplicator(str, type, btn) {
    ...
    if ($debug === true) {
      console.warn("Duplicator: debug mode is enabled, no redirect.");
      return;
    }
    ...
    switch (type) {
      ...
      case "packageCreated":
        href = $(btn)
          .data("action")
          .replace("?action=backup_now", "?action=none");
        break;

      case "deleteAll":
        href = $(btn)
          .data("action")
          .replace("?action=deleteAll", "?action=none");
        break;

      default: //.replace('?action=backup_now', '?action=' + type + '&msg=' + encoded);
        href = $(btn).data("action");
        break;
    }
